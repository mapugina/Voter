...
voteS.factory('PlayerlistService', ['$cookieStore', '$http', '$q',
  function ($cookieStore, $http, $q) {
    ...
    instance.loadDefaults = function (deferred) {
      $http.get('defaults.json').success(function (array) {
        instance.players = array;
        ...
        if (deferred !== undefined && deferred !== null) {
          deferred.reject('Failed to load defaults');
        }
      });
    };
